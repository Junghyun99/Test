...
def test_full_scenario(moni_db):
    ...
    assert len(result) == 1
    assert result[0][2] == 'AAPL'
    assert result[0][6] == 15  # Updated amount
...
def sample_data(moni_db):
    # 각 국가별 모니터링내역 5개씩 생성
    kr_moni = [
        ('삼성전자', '005390', 'KR', 1, 60000, 5, 6),
        ('현대차', '005830', 'KR', 3, 240000, 1, 10),
        ('네이버', '002380', 'KR', 1, 6000, 10, 23),
        ('카카오', '005530', 'KR', 5, 120000, 3, 4),
        ('하이브', '012880', 'KR', 3, 6200, 14, 5)
    ]
    us_moni = [
        ('apple', 'AAPL', 'US', 1, 180.4, 5, 10),
        ('amazon', 'AMZN', 'US', 2, 432.1, 8, 9),
        ('tesla', 'TSLA', 'US', 5, 300.8, 2, 11),
        ('micro soft', 'MSFT', 'US', 2, 150.7, 10, 32),
        ('reality income', 'O', 'US', 3, 90, 5, 1)
    ]                 
